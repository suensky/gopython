...
# Sundar Pichai became CEO of Google on October 2, 2015
start_date = "2015-10-02"
end_date = "2023-11-21"

# Downloading Google stock data (Alphabet Inc.)
...
start_date_adj = googl.loc[start_date_ts:end_date_ts].first_valid_index()
end_date_adj = googl.loc[start_date_ts:end_date_ts].last_valid_index()

# Get the closing price for the adjusted start and end dates
start_price_point = googl.loc[start_date_adj, 'Close']
current_price_point = googl.loc[end_date_adj, 'Close']

# Plotting the closing price of Google stock
plt.figure(figsize=(14,7))
plt.plot(googl['Close'], label='GOOGL Closing Price')
...
plt.title('Google Stock Performance Since Sundar Pichai Became CEO')
plt.xlabel('Date')
